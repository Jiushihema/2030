...
{YELLOW}========== 攻击注入控制台 =========={RESET}
  {RED}[1-1]{RESET} 持续过压帧（仅合闸时生效；分闸后为失电）
  {RED}[1-2]{RESET} 篡改传感器位置为 open   (trip 指令被拒 → 线路持续带故障)
  {RED}[3-1]{RESET} 通信干扰（间隔-站控）
  {RED}[3-2]{RESET} 通信干扰（断路器-间隔）
  {RED}[4]{RESET} 授时欺骗（断路器）
  {RED}[5]{RESET} 伪造间隔层 GOOSE 直投合闸 + 闭锁过压跳闸/自动重合闸
  {RED}[o]{RESET} 电网过载异常
  {GREEN}[m-1]{RESET} 人工合闸
  {GREEN}[m-2]{RESET} 人工分闸
...
CMD_HINTS = {
    ...
    "3-2": "通信干扰（断路器-间隔）",
    "4": "授时欺骗（断路器）",
    "5": "伪造间隔层 GOOSE 直投合闸 + 闭锁过压跳闸/自动重合闸",
    "o": "电网过载异常",
    "m-1": "人工合闸",
    "m-2": "人工分闸",
    "r": "重置所有状态",
