...
export const ATTACK_TIMELINES = {
  ...
  alarm: {
    gatewayA: [
      ...
      // { nodes: ['mechanical_sensor'], edges: ['e_sensor_bit'], summary: '异常先从现场状态上传链路出现。', consequences: ['现场状态虽然变化，但上层难以形成有效告警。'] },
      // { nodes: ['mechanical_sensor', 'breaker_it'], edges: ['e_sensor_bit', 'e_bit_lm'], summary: '告警语义在设备侧被弱化。', consequences: ['断路器状态异常没有形成足够强的提醒。'] },
      { nodes: ['breaker_it', 'line_monitor'], edges: ['e_bit_lm', 'e_lm_bit'], summary: '线路测控与断路器间的通讯被阻断。', consequences: ['控制指令无法下发。'] },
    ],
    gatewayB: [
      ...
      { nodes: ['line_monitor', 'monitor_host'], edges: ['e_lm_mh', 'e_mh_lm'], summary: '线路测控与监控主站间的通讯被阻断。', consequences: ['告警压制影响数据留痕，后续追溯时证据完整性下降。'] },
    ],
  },
  timing: {
    auto: [
      { nodes: ['time_sync'], edges: [], summary: '攻击者使用仿冒时钟源。', consequences: ['部分设备时钟出现偏移。'] },
      // { nodes: ['time_sync'], edges: ['e_ts_mh', 'e_ts_lmu', 'e_ts_bit'], summary: '偏移时标向关键节点扩散。', consequences: ['多个节点开始收到异常时间戳。'] },
      { nodes: ['time_sync'], edges: ['e_ts_bit'], summary: '偏移时标向关键节点扩散。', consequences: ['指定节点开始收到异常时间戳。'] },
      { nodes: ['time_sync', 'breaker_it'], edges: ['e_ts_bit'], summary: '关键控制与测量节点同时受扰。', consequences: ['联动顺序与事件先后关系变得不可靠。'] },
      { nodes: ['time_sync', 'line_monitor', 'breaker_it'], edges: ['e_ts_bit', 'e_lm_bit', 'e_bit_lm'], summary: '全局态势受到时标偏移波及。', consequences: ['运行全景对事件因果的判断被严重干扰。'] },
    ],
  },
  swarm: {
    auto: [
